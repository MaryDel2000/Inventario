...
public interface InvProductoRepository extends JpaRepository<InvProducto, Long> {
    @org.springframework.data.jpa.repository.Query("select distinct p from InvProducto p " +
           "where (?1 is null or p.categoria = ?1) " +
           "and (?2 is null or p.unidadMedida = ?2) " +
           "and (?3 is null or p.activo = ?3) " +
           "and (?4 is null or exists (select 1 from InvExistencia e where e.productoVariante.producto = p and e.almacen = ?4))")
