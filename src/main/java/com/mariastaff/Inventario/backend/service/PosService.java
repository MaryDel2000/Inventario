...
public class PosService {
    ...
    private final com.mariastaff.Inventario.backend.data.repository.PosPagoRepository pagoRepository;
    ...
    public PosService(PosVentaRepository ventaRepository, PosTurnoRepository turnoRepository,
            PosCajaRepository cajaRepository, PosClienteRepository clienteRepository,
            com.mariastaff.Inventario.backend.data.repository.PosPagoRepository pagoRepository,
    ...
    public PosTurno findOpenShiftForUser(com.mariastaff.Inventario.backend.data.entity.SysUsuario user) {
        if (user == null)
            return null;
        return turnoRepository.findAll().stream()
                .filter(t -> "ABIERTO".equals(t.getEstado()) && t.getUsuarioCajero() != null
                        && t.getUsuarioCajero().getId().equals(user.getId()))
                .findFirst()
                .orElse(null);
    }
    ...
    public List<PosVenta> findAllVentasWithDetails() {
        return ventaRepository.findAllWithDetails();
    }

    public void deleteVenta(PosVenta entity) {
        ventaRepository.delete(entity);
    }

    public List<PosVenta> findVentasPorCobrar() {
        return ventaRepository.findByEstadoPagoNot("PAGADO");
    }

    public List<PosTurno> findAllTurnos() {
        return turnoRepository.findAll();
    }

    public PosTurno saveTurno(PosTurno entity) {
        return turnoRepository.save(entity);
    }

    public void deleteTurno(PosTurno entity) {
        turnoRepository.delete(entity);
    }

    public List<PosCaja> findAllCajas() {
        return cajaRepository.findAll();
    }

    public PosCaja saveCaja(PosCaja entity) {
        return cajaRepository.save(entity);
    }

    public void deleteCaja(PosCaja entity) {
        cajaRepository.delete(entity);
    }

    public List<PosCliente> findAllClientes() {
        return clienteRepository.findAll();
    }

    public PosCliente saveCliente(PosCliente entity) {
        return clienteRepository.save(entity);
    }

    public com.mariastaff.Inventario.backend.data.entity.PosPago savePago(
            com.mariastaff.Inventario.backend.data.entity.PosPago entity) {
        return pagoRepository.save(entity);
    }

    public void deletePago(com.mariastaff.Inventario.backend.data.entity.PosPago entity) {
    ...
    public PosVenta saveVenta(PosVenta entity) {
        ...
        if (entity.getAlmacenSalida() != null && "CERRADO".equals(entity.getEstado())) {
            ...
            for (com.mariastaff.Inventario.backend.data.entity.PosVentaDetalle detalle : entity.getDetalles()) {
                ...
                // Find all stock for this variant in the source warehouse, ordered by expiry or
                // modification (FIFO)
                // Simplification: just find by variant, filter by warehouse. Ideally repo has a
                // method.
                List<com.mariastaff.Inventario.backend.data.entity.InvExistencia> stockList = existenciaRepository
                        .findByProductoVariante(detalle.getProductoVariante());

                // Sort by: Expiry Date (Asc), then Updated Date (Asc)
                stockList.sort((e1, e2) -> {
                    if (e1.getLote() != null && e2.getLote() != null && e1.getLote().getFechaCaducidad() != null
                            && e2.getLote().getFechaCaducidad() != null) {
                ...
                for (com.mariastaff.Inventario.backend.data.entity.InvExistencia stock : stockList) {
                    if (!stock.getAlmacen().getId().equals(entity.getAlmacenSalida().getId()))
                        continue;
                    if (stock.getCantidadDisponible().compareTo(java.math.BigDecimal.ZERO) <= 0)
                        continue;
                ...
                // If there is still remaining quantity, forces a deduction from a default
                // location or generic
                // (or we just log/warn. Here we force a negative on the first match or null
                // lote to ensure movement is recorded)
                if (qtyRemaining.compareTo(java.math.BigDecimal.ZERO) > 0) {
                    com.mariastaff.Inventario.backend.data.entity.InvMovimientoDetalle movDet = new com.mariastaff.Inventario.backend.data.entity.InvMovimientoDetalle();
                    movDet.setProductoVariante(detalle.getProductoVariante());
    ...
    public void deleteCliente(PosCliente entity) {
    ...
    public java.util.Map<String, java.math.BigDecimal> getSalesByTurno() {
        ...
        for (PosTurno turno : closedTurns) {
            ...
            String label = (turno.getFechaHoraCierre() != null ? turno.getFechaHoraCierre().format(formatter)
                    : "Turno " + turno.getId());
            map.put(label, total);
        }
        return map;
