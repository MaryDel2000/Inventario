...
./gradlew clean vaadinBuildFrontend bootWar -Pproduction
...
if [ $? -eq 0 ]; then
    ...
    # Copiar el WAR renombrándolo a ROOT.war para que sea la aplicación raíz (localhost:8080)
    # Si prefieres un context path específico (ej. localhost:8080/mi-app), cambia ROOT.war por mi-app.war
